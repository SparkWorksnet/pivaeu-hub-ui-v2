...
import { computed, shallowReactive, toValue, provide } from 'vue'
...
<template>
  <div>
    ...
    <DetailsPage :headline="t('dataset.title')" :title="resultEnhanced?.getTitle || ''"
      ...
      <template #subtitle="{ subtitle }">
        <RouterLink :to="{ name: 'Datasets', query: { catalog: resultEnhanced?.getCatalogId } }" class="
            by-link
          ">
          {{ subtitle }}
        </RouterLink>
      ...
      <template #sections>
          ...
          <div name="distribution-cards" class="relative">
            <template v-for="(distribution, i) in truncatedFormattedDistributions" :key="distribution.id">
              <div name="distribution-card-wrapper" class="relative">
                <DistributionCard :title="distribution.title || ''" :description="distribution.descriptionMarkup || ''"
                ...
                <div v-if="i === truncatedFormattedDistributions.length - 1 && isDistributionsTruncated"
            ...
            <!-- Show Less-->
            <div v-if="!isDistributionsTruncated && getFormattedDistributions.length > 7"
        ...
        <div v-if="(resultEnhanced?.getCategories?.length || 0) > 0" class="space-y-3">
          ...
          <div class="flex flex-row gap-2 mt-5">
            <KTag v-for="category in resultEnhanced?.getCategories" :key="category.id" interactive
        ...
        <div class="space-y-4 mt-6 mb-8">
          <div class="flex flex-col rounded-xl gap-4 bg-surface p-4">
            ...
            <PropertyTable v-if="isSuccess" :node="{
              type: 'node',
              id: 'a',
              label: 'a',
